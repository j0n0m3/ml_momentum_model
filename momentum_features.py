...
tickers = pd.read_sql("SELECT * FROM all_assets", con=engine)["ticker"].values
...
trading_calendar = (
    calendar.schedule(
        start_date=full_grouped_bars.index[0].strftime("%Y-%m-%d"),
...
for first_day in first_trading_days:
    ...
    day_data = full_grouped_bars[
        full_grouped_bars.index.date == pd.to_datetime(first_day).date()
    ].copy()

    # monthly_data = historical_data.groupby("ticker").resample("M").sum(numeric_only=True).reset_index()

    # get top tickers by notional volume (amount traded)
    ticker_count = 2000
    day_data["notional_volume"] = day_data["vw"] * day_data["v"]
    day_data = day_data.sort_values("notional_volume", ascending=False).head(
        ticker_count
    )
    ...
    for ticker in trading_tickers:
        ...
        monthly_returns = round(
            ((last_day_close - first_day_close) / first_day_close) * 100, 2
        )
        ...
        full_ticker_data = full_grouped_bars[
            full_grouped_bars["ticker"] == ticker
        ].copy()
        trading_day_data = full_ticker_data[
            full_ticker_data.index.date == pd.to_datetime(first_day).date()
        ]
        selling_day_data = full_ticker_data[
            full_ticker_data.index.date == pd.to_datetime(last_day).date()
        ]

        if (len(trading_day_data) < 1) or (len(selling_day_data) < 1):
            continue

        momentum_dataframe = pd.DataFrame(
            [
                {
                    "date": pd.to_datetime(first_day),
                    "ticker": ticker,
                    "last_n_mo_avg_returns": last_n_months_avg_return,
                    "last_n_mo_avg_returs_mean": last_n_months_avg_return_mean,
                    "return_std": return_std,
                    "return_before_last_month": return_before_last,
                    "last_month_return": last_return,
                    "trade_price": trading_day_data["c"].iloc[0],
                    "pct_change": round(
                        (
                            (
                                selling_day_data["c"].iloc[0]
                                - trading_day_data["c"].iloc[0]
                            )
                            / trading_day_data["c"].iloc[0]
                        )
                        * 100,
    ...
    momentum_data_og = pd.concat(momentum_data_list).sort_values(
        by="last_n_mo_avg_returns", ascending=True
    )

    momentum_data = momentum_data_og.copy()
    ...
    iteration = round(
        (np.where(first_trading_days == first_day)[0][0] / len(first_trading_days))
        * 100,
        2,
    )
    iterations_remaining = (
        len(first_trading_days) - np.where(first_trading_days == first_day)[0][0]
    )
    ...
    print(
        f"complete: {iteration}%, remaining time: {time_remaining}, eta: {estimated_completion_time}"
    )
